	...
	public function get_form() { ?>
		<div class="postbox">
			...
			<div class="inside">
				...
				<ul class="yfym_ul">
					<li><?php _e('URL your feeds', 'yfym'); ?></li>
					<li><?php _e('Files generation status', 'yfym'); ?></li>
					<li><?php _e('PHP version information ', 'yfym'); ?></li>
					<li><?php _e('Multisite mode status', 'yfym'); ?></li>
					<li><?php _e('Technical information and plugin logs', 'yfym'); ?> YML for Yandex Market</li>
				</ul>
				<p><?php _e('Did my plugin help you upload your products to the', 'yfym'); ?> YML for Yandex Market?</p>
				<form action="<?php echo esc_url($_SERVER['REQUEST_URI']); ?>" method="post" enctype="multipart/form-data">
					<p>
						<input type="radio" name="<?php echo $this->get_radio_name(); ?>" value="yes"><?php _e('Yes', 'yfym'); ?><br />
						<input type="radio" name="<?php echo $this->get_radio_name(); ?>" value="no"><?php _e('No', 'yfym'); ?><br />
					</p>
					...
					<p><textarea rows="6" cols="32" name="<?php echo $this->get_textarea_name(); ?>" placeholder="<?php _e('Enter your text to send me a message (You can write me in Russian or English). I check my email several times a day', 'yfym'); ?>"></textarea></p>
					<?php wp_nonce_field($this->get_nonce_action(), $this->get_nonce_field()); ?>
					<input class="button-primary" type="submit" name="<?php echo $this->get_submit_name(); ?>" value="<?php _e('Send data', 'yfym'); ?>" />
				</form>	
			</div>
		</div><?php
	}
		...
		<div class="postbox">
			...
			<div class="inside">	  
				...
				<ul class="yfym_ul">
					...
					<li><?php _e('Support the project financially', 'yfym'); ?>. <a href="//pay.cloudtips.ru/p/45d8ff3f" target="_blank"> <?php _e('Donate now', 'yfym'); ?></a>.</li>
	...
	private function listen_submits_func() {
		if (isset($_REQUEST[$this->get_submit_name()])) {
			...
			add_action('admin_notices', function() {
				...
				$message = __('The data has been sent. Thank you', 'yfym');
